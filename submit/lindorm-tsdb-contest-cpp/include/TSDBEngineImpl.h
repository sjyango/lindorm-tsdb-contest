...
//        std::shared_mutex &_get_mutex_for_vin_timestamp_range(const Vin &vin, int64_t range);
    ...
    // 0 ~ 3599
    static int32_t get_timestamp_num(const int64_t timestamp) {
    ...
    static int32_t combine_vin_and_timestamp(const Vin &vin, const int64_t timestamp) {
        ...
        int32_t timestamp_range_num = get_timestamp_num(timestamp) / VIN_TIME_RANGE_WIDTH; // 0 ~ VIN_TIME_RANGE_NUM

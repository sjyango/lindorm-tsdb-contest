...
namespace LindormContest::compression {
    ...
    uint32_t CompressionCodecGorilla::compress(const char *source, uint32_t source_size, char *dest) const {
        ...
        switch (data_bytes_size) {
            case 1:
                result_size = compressDataForType<uint8_t>(&source[bytes_to_skip], source_size - bytes_to_skip,
                                                           &dest[start_pos], compressed_size);
                break;
            case 2:
                result_size = compressDataForType<uint16_t>(&source[bytes_to_skip], source_size - bytes_to_skip,
                                                            &dest[start_pos], compressed_size);
                break;
            case 4:
                result_size = compressDataForType<uint32_t>(&source[bytes_to_skip], source_size - bytes_to_skip,
                                                            &dest[start_pos], compressed_size);
                break;
            case 8:
                result_size = compressDataForType<uint64_t>(&source[bytes_to_skip], source_size - bytes_to_skip,
                                                            &dest[start_pos], compressed_size);
                break;
        }
    ...
    uint8_t CompressionCodecGorilla::decompress(const char *source, uint32_t source_size, char *dest,
        ...
        switch (bytes_size) {
            case 1:
                ...
                break;
            case 2:
                decompressDataForType<uint16_t>(&source[2 + bytes_to_skip], source_size_no_header,
                                                &dest[bytes_to_skip]);
                break;
            case 4:
                decompressDataForType<uint32_t>(&source[2 + bytes_to_skip], source_size_no_header,
                                                &dest[bytes_to_skip]);
                break;
            case 8:
                decompressDataForType<uint64_t>(&source[2 + bytes_to_skip], source_size_no_header,
                                                &dest[bytes_to_skip]);
                break;
        }

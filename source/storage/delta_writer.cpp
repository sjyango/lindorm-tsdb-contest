...
DeltaWriter::DeltaWriter(const String& table_name, const Schema& schema)
    ...
    _mem_table = std::make_unique<MemTable>(_schema.get(), _next_segment_id++);
}
...
std::optional<SegmentData> DeltaWriter::flush() {
    ...
    SegmentData segment_data = std::move(_mem_table->finalize());
    _mem_table = std::make_unique<MemTable>(_schema.get(), _next_segment_id++); // reset mem_table
    return segment_data;
}

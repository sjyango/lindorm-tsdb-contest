...
uint32_t CompressionCodecGorilla::compress(const LindormContest::Slice& input, std::string* output) {
    return doCompressData(input.data(), input.size(), output->data());
}
uint8_t CompressionCodecGorilla::decompress(const LindormContest::Slice& input, LindormContest::Slice* output) {
    auto uncompressed_size = output->size();
    auto byte_skip = doDecompressData(input.data(),input.size(),output->mutableData(), uncompressed_size);
    INFO_LOG("byte skip : %d ", byte_skip)
    return byte_skip;
}
uint32_t CompressionCodecGorilla::doCompressData(const char * source, uint32_t source_size, char * dest) const
{
...
uint8_t CompressionCodecGorilla::doDecompressData(const char * source, uint32_t source_size, char * dest, uint32_t uncompressed_size) const
{
    if (source_size < 2)
    ...
    if (static_cast<uint32_t>(2 + bytes_to_skip) > source_size)
        throw std::runtime_error("Cannot decompress. File has wrong header");
    // fixme(hch): we delete the bytes_to_skip data which seems like no use.

...
TSDBEngineImpl::~TSDBEngineImpl() {
    ...
    for (auto& table : _tables) {
        table.second->_latest_records.clear();
        table.second->_table_schema.reset();
        table.second->_table_writer.reset();
        table.second->_table_reader.reset();
        table.second->_fs.reset();
    }
    _tables.clear();
}
...
int TSDBEngineImpl::connect() {
    ...
    for (const auto& file_info : file_infos) {
        ...
        table->_latest_records = std::move(load_latest_records_from_file(table->_table_schema, table_path / "latest_records.dat"));
...
int TSDBEngineImpl::shutdown() {
    for (const auto& table : _tables) {
        ...
        if (flushed_records.has_value()) {
            update_latest_records(table.second->_latest_records, *flushed_records);
        ...
        save_latest_records_to_file(table.second->_table_schema, table.second->_latest_records, latest_records_path);
    }
...
int TSDBEngineImpl::upsert(const WriteRequest &writeRequest) {
    ...
    }
    {
        ...
        if (flushed_records.has_value()) {
            ...
            update_latest_records(table->_latest_records, *flushed_records);
...
int TSDBEngineImpl::executeLatestQuery(const LatestQueryRequest &pReadReq, std::vector<Row> &pReadRes) {
    ...
    }
    {
        ...
        for (const auto& vin : pReadReq.vins) {
            ...
            Row latest_row = table->_latest_records[vin_val];
            Row result_row;
            result_row.vin = latest_row.vin;
            result_row.timestamp = latest_row.timestamp;
            std::map<std::string, ColumnValue> columns;

            for (const auto& column : pReadReq.requestedColumns) {
                columns.emplace(column, latest_row.columns[column]);
            }

            result_row.columns = std::move(columns);
            pReadRes.emplace_back(result_row);
        }
    }
    return 0;
}
